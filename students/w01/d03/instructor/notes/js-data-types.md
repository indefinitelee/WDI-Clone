...
_Student facing notes adapted from [Gerry Mathe's JS Data types, Variables and Arrays lesson](https://github.com/GA-WDI/curriculum/tree/master/resources/00-programming/js-data-types-variables-and-arrays)_
...
- Most basic form of data in JavaScript
- Data that is not an object or a function

### Boolean
- True/False

### String
- Represents textual data. In JS strings are immutable, meaning you cannot
modify strings in JavaScript.

### Number
- In JS there is only one Number type, which is double precision 64 bit binary format, ranges from `-(2^53 - 1)` and `(2^53 - 1)`. There is no specific type for integers.
  * Other values in the Numbers type: `Infinity`, `-Infinity`, `NaN`
...
5 / 2
=> 2.5
// Jump into ruby to show that this treats everything as a float


// Show some precision issues with:
0.2 + 0.1
// What could possibly be happening
// The way they are stored in the computer creates a precision error

.99999999999 => rounds to 1

// We can fix this by just moving the decimal
// BE CAREFUL WITH ADDITION OF FLOATING POINT NUMBERS IN JS ESPECIALLY FOR
// ADDING UP MONEY
...
"hello".length;
=> 5
// This is very odd behavor for a primitive data type - this breaks the rules
```
...
```
```javascript
"Will this do anything good?" - 1
// Results in NaN
```

*Go to `js_strings.md` and go through strings activity*
...
- Draw out the Truth Tables for both `&&` and `||`
